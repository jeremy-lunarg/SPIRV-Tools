...
class RemapIdsPass : public Pass {
  ...
  private:
    // TODO: Verify that variables and functions are named in accordance with the Google style
    // guide.
    ...
    spv::Id Bound() const; // all <id>s are guaranteed to satisfy 0 < id < bound.
    inline spv::Id NextUnusedId(spv::Id id); // return next unused new id.




    // implement a simple dynamic bitset to track which IDs have been mapped
    typedef std::uint64_t bits_t;
    std::vector<bits_t> mapped_; // which new IDs have been mapped
    // TODO: JJH - Why are half the bits unused? The 4 should be an 8.
    ...
    bool IsOldIdUnmapped(spv::Id old_id) const { return LocalId(old_id) == unmapped_; }
    bool IsOldIdUnused(spv::Id old_id) const { return LocalId(old_id) == unused_;   }
    bool IsOldIdMapped(spv::Id oldId) const { return !IsOldIdUnused(oldId) && !IsOldIdUnmapped(oldId); }

    // TODO: JJH - I want to call these new_ids_
    // a mapping from old ids to new ids e.g. id_map_l_[old_id] = new_id
    std::vector<spv::Id> id_map_l_;
    // get local id
    // TODO: JJH - I want to call this NewId(...)
    spv::Id LocalId(spv::Id id) const { return id_map_l_[id]; }
    // set local id
    // TODO: JJH - I want to call this NewId(...)
    spv::Id LocalId(spv::Id id, spv::Id new_id = unmapped_);

    spv::Id HashType(spv::Id id) const;
    ...
    spv::Id largest_new_id_; // biggest new ID we have mapped anything to
