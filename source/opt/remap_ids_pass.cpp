...
Pass::Status RemapIdsPass::Process() {
  ...
  // TODO: add function to verify that all ids are mapped (i.e. not unmapped_)
...
void RemapIdsPass::RemapTypeAndConst(){
  ...
  for (auto const id : type_and_const_ids_) {
    ...
    auto const hash_value = (HashTypeAndConst(id) % softTypeIdLimit) + firstMappedID;

    if (IsOldIdUnmapped(id)) {
      SetNewId(id, NextUnusedId(hash_value));
    }
  }
}
...
spv::Id RemapIdsPass::HashTypeAndConst(spv::Id id) const
{
  spv::Id value = 0;
...
void RemapIdsPass::RemapFunctions() {
  ...
  static constexpr std::uint32_t first_mapped_id   =  6203;  // offset into ID space
  ...
  for (auto const func_id : function_ids_) {
    // {
    //   // create a temporary vector to store the instructions so they can be easily referenced
    ...
    //     for (int32_t j = lower_bound; j < i; ++j) {
    //       auto const local_inst = insts[j];
    //       if (local_inst->opcode() == spv::Op::OpFunction) {
    //         break;
    //       }
    //       hash_value = hash_value * 30103 + HashOpCode(local_inst); // 30103 = semiarbitrary prime
    //     }
    ...
    //     SetNewId(old_id, NextUnusedId(hash_value % soft_type_id_limit + first_mapped_id));
    //   }
    // }

    // store the instructions and opcode hash values in vectors so that the window of instructions
    // can be easily accessed and avoid having to recompute the hash value repeatedly in
    // overlapping windows
    std::vector<Instruction*> insts;
    ...
    for (int32_t i = 0; i < (int32_t)insts.size(); ++i) {
      ...
      for (int32_t j = i - 1; j >= lower_bound; --j) {
        ...
        hash_value = hash_value * 30103 + opcode_hashvals[j]; // 30103 = semiarbitrary prime
      }
...
spv::Id RemapIdsPass::HashOpCode(Instruction* inst) const {
  auto const op_code = inst->opcode();
...
void RemapIdsPass::RemapRemainders() {
  for (auto const id : remainder_ids) {
    spv::Id new_id = 1;
    if (IsOldIdUnmapped(id)) {
      SetNewId(id, new_id = NextUnusedId(new_id));
    }
  }
}

void RemapIdsPass::ApplyMap() {
  // TODO: do we need to invalidate the DebugInfo analysis? See compact_ids_pass.cpp.

  context()->module()->ForEachInst([this](Instruction* inst) {
    for(auto operand = inst->begin(); operand != inst->end(); ++operand) {
      const auto type = operand->type;
      if(spvIsIdType(type)) {
        uint32_t& id = operand->words[0];
        uint32_t const new_id = GetNewId(id);
        // TODO: we shouldn't care about unused; let other passes handle that
        assert(new_id != unmapped_ && new_id != unused_);
        if (id != new_id) {
          id = new_id;
...
void RemapIdsPass::ReplaceId(spv::Id old_id, spv::Id new_id) {
  ...
  inst->SetResultId(old_id);
  get_def_use_mgr()->AnalyzeInstDef(inst);

  context()->ReplaceAllUsesWith(old_id, old_id);
...
// TODO: remove all this (from naive port attempt)
