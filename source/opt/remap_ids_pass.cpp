...
void RemapIdsPass::RemapFunctions() {
  ...
  for (auto const func_id : function_ids_) {
    // context()->module()->ForEachInst([this, &hash_value](Instruction* inst) {
    // }

    // auto const inst = get_def_use_mgr()->GetDef(func_id);
    // (void) inst;

    // create a temporary map to store the aggregate sum over the window of instructions
    std::unordered_map<spv::Id, spv::Id> hash_map;

    // initialize with starting hash value
    spv::Id hash_value = func_id * 17; // small prime
    auto const func = context()->GetFunction(func_id);
    func->WhileEachInst([&](Instruction* inst) {
      hash_map[inst->result_id()] = hash_value;
...
void RemapIdsPass::ApplyMap() {
  ...
  // TODO: ForEachInst takes an argument that runs on debug lines. Should this be true?
  ...
  context()->module()->ForEachInst([this](Instruction* inst) {
    ...
    // for (uint32_t w=2; w < inst->NumOperandWords(); ++w) {
    // for(uint32_t op_i = 0; op_i < inst->NumOperands(); ++op_i) {
    //   (void) op_i;
    // }
  
  }, true);

  // this is try #2. remove.
  // {
  // context()->module()->ForEachInst([this](Instruction* inst) {
  //   if (inst->HasResultId()) {
  //     auto const id = inst->result_id();
  //     auto const new_id = GetNewId(id);
  //     if (id != new_id) {
  ...
  // TODO: apply name maps
  // TODO: apply function
  // TODO: apply remainder maps
  // }
}
